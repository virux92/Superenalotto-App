...
def normalize_archive_dataframe(raw_dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # Protezione esplicita dal caso in cui una riga di intestazione sia stata letta come dato.
    ...
    if duplicate_contests.any():
        ...
        details = ", ".join(f"{row.anno}/Conc.{row.concorso}" for row in duplicated.itertuples())
    ...
    for row_number, row in enumerate(dataframe.itertuples(index=False), start=2):
        if row.data.year != row.anno:
            raise ValueError(
                f"Anno incoerente alla riga {row_number}: data {row.data:%d/%m/%Y}, anno indicato {row.anno}."
            )
...
def load_repository_archive(path_text: str) -> pd.DataFrame:
    path = Path(path_text)
    if not path.exists():
        raise FileNotFoundError("File estrazioni.csv non trovato. Caricalo nella stessa cartella di app.py.")
    return read_csv_flexible(path)


@st.cache_data(show_spinner=False, ttl=300)
def load_primary_archive(path_text: str, _database_loader: Callable[[], pd.DataFrame]) -> tuple[pd.DataFrame, str, str | None]:
    ...
    except Exception as exc:  # fallback deliberato: l'app deve restare disponibile
